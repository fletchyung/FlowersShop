...
public class dashboardController implements Initializable {
    ...
    public void homeAF() {
        ...
        String sql = "SELECT COUNT(id) FROM flowers WHERE status = 'Доступно'";
        ...
        try {
            ...
            if (availableFlowers_flowerID.getText().isEmpty()
                ...
                if (result.next()) {
                    ...
                    alert.setTitle("Ошибка!");
                    alert.setHeaderText(null);
                    alert.setContentText("Цветок с ID: " + availableFlowers_flowerID.getText() + " уже существует!");
                    alert.showAndWait();
                } else {
                    prepare = connect.prepareStatement(sql);
    ...
    public void availableFlowersUpdate() {
        ...
        try {
            ...
            if (availableFlowers_flowerID.getText().isEmpty()
                ...
                alert.setHeaderText(null);
                alert.setContentText("Вы уверены, что хотите обновить  цветок с ID: " + availableFlowers_flowerID.getText() + "?");
    ...
    public void availableFlowersDelete() {
        ...
        try {
            ...
            if (availableFlowers_flowerID.getText().isEmpty()
                ...
                alert.setTitle("Сообщение с подтверждением");
                alert.setHeaderText(null);
                alert.setContentText("Вы уверены, что хотите удалить цветок с ID: " + availableFlowers_flowerID.getText() + "?");
                Optional<ButtonType> option = alert.showAndWait();
    ...
    String listStatus[] = {"Доступно", "Не доступно"};
    ...
    public void availableFlowersInsertImage() {
        ...
        open.setTitle("Открыть файл изображения");
    ...
    public void purchaseAddToCart() {
        ...
        try {
            ...
            if (purchase_flowerID.getSelectionModel().getSelectedItem() == null
                ...
                alert.setTitle("Ошибка!");
                alert.setHeaderText(null);
                alert.setContentText("Пожалуйста, сначала выберите продукт");
                alert.showAndWait();
            } else {
                double priceData = 0;
                double totalPrice;
        ...
        try {
            ...
            if (totalP == 0) {
                ...
                if (option.get().equals(ButtonType.OK)) {
                    ...
                    alert.setTitle("Информационное сообщение");
                    alert.setHeaderText(null);
                    alert.setContentText("Успешно! Спасибо за покупку");
                    alert.showAndWait();
